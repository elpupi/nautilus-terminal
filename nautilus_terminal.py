...
class NautilusTerminal(object):
    ...
    def _build_and_inject_ui(self):
        ...
        fg_color = settings_fg_color if settings_fg_color else "rgb(255, 255, 255)"
        bg_color = settings_bg_color if settings_bg_color else  "rgb(0, 0, 0)"
        ...
        if self._settings.get_boolean("use-gnome-terminal-palette"):
            profiles = self.settings_try_new("org.gnome.Terminal.ProfilesList")

            if profiles:
                default_path = "/org/gnome/terminal/legacy/profiles:/:" + profiles.get_string("default") + "/"
                settings = Gio.Settings.new_with_path("org.gnome.Terminal.Legacy.Profile", default_path)
                palette_string = settings.get_strv("palette")
            else:
                palette_string = []
        else:
            palette_string = self._settings.get_strv("palette")


        for str_color in palette_string:
            try:
                palette.append(makeRGBA(str_color))
            except:
                palette = []
                break
        ...
        #         # (52, 226, 226, 1),
        #         # (238, 238, 236, 1)
        #         "#3b4252", "#bf616a", "#2aa0ba", "#d4c726", "#21a3db", "#b48ead", "#88c0d0", "#e5e9f0",
        #         "#4c566a", "#bf616a", "#a3be8c", "#c4a000", "#81a1c1", "#b48ead", "#8fbcbb", "#eceff4"
        #     ]

        # palette = []

        # for color in palette_def:
        #     rgba = color_helpers.parse_color_string(color)
        #     palette.append(makeRGBA(rgba))

        self._ui_terminal.set_colors(
            makeRGBA(fg_color),
            makeRGBA(bg_color),
            palette
        )
